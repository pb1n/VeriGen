...
namespace veri
{
    ...
    public:
        ...
        Generator(unsigned seed, int min_s=0, int max_s=0, int min_i=2, int max_i=16, bool rand_up=true) 
            : rng(seed), seed_(seed), min_start(min_s), max_start(max_s), min_iter(min_i), max_iter(max_i), random_update(rand_up) {}
        ...
        make(const std::string &topName, int idx, int depth = 2)
        {
            ...
            f << "// auto-generated by VeriGen-Generate \n // seed: " << seed_ << "\n`timescale 1ns/1ps\n\n";
        ...
        makeModule(const std::string& uniq, int depth = 2) {
            auto [path, val] = make(uniq, 0, depth > 3 ? 3 : depth);
            (void)path;
