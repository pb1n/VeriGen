...
void saveModule(const ast::Ptr<ast::ModuleDefn>& module, const std::string& filename, const std::string& description) {
    ...
    std::string verilog = module->emit();
    file << verilog;
    file.close();
...
int main() {
    ...
    IcarusSimulator sim("sim_work", false);
    ...
    saveModule(counter1, "basic_counter.v", "Basic Counter WITH reset");
    simulateCounter(sim, "basic_counter.v", "basic_counter", 8, true, false);

    // Test counter without reset
    auto counter2 = generators::generateCounter("simple_counter", 16, false);
    saveModule(counter2, "simple_counter.v", "Basic Counter WITHOUT reset");
    simulateCounter(sim, "simple_counter.v", "simple_counter", 16, false, false);

    // Test counter with reset and load
    auto counter3 = generators::generateCounter("loadable_counter", 8, true, true);
    saveModule(counter3, "loadable_counter.v", "Counter WITH reset AND load");
    simulateCounter(sim, "loadable_counter.v", "loadable_counter", 8, true, true);

    // Test counter with load but no reset
    auto counter4 = generators::generateCounter("loadable_no_reset", 8, false, true);
    saveModule(counter4, "loadable_no_reset.v", "Counter WITH load, WITHOUT reset");
    simulateCounter(sim, "loadable_no_reset.v", "loadable_no_reset", 8, false, true);

    // Test down-counter with reset and load
    auto counter5 = generators::generateCounter("down_counter", 8, true, true, true);
    saveModule(counter5, "down_counter.v", "Down-Counter WITH reset AND load");
    simulateCounter(sim, "down_counter.v", "down_counter", 8, true, true);

    // Test down-counter without reset or load
    auto counter6 = generators::generateCounter("simple_down", 8, false, false, true);
    saveModule(counter6, "simple_down.v", "Down-Counter WITHOUT reset or load");
    simulateCounter(sim, "simple_down.v", "simple_down", 8, false, false);
